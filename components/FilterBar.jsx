...
export default function FilterBar({ filters, setFilters, view, setView, resultCount, loading }) {
  ...
  const timeframeLabel = TIMEFRAMES.find(t => t.id === filters.timeframe)?.label;
    ...
    <div ref={panelRef} className="sticky top-16 z-30 -mx-4 sm:-mx-6 px-4 sm:px-6 py-3 bg-black/85 backdrop-blur-lg border-b border-zborder">
      ...
      <div className="flex items-center gap-2">
        ...
        <div className="hidden sm:inline-flex bg-zcard border border-zborder rounded-full p-0.5 shrink-0">
          {['list', 'map'].map(v => (
            ...
            >
              {v === 'list' ? (
                <span className="inline-flex items-center gap-1"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="M8 6h13M8 12h13M8 18h13"/><circle cx="4" cy="6" r="1.5"/><circle cx="4" cy="12" r="1.5"/><circle cx="4" cy="18" r="1.5"/></svg>List</span>
              ) : (
                <span className="inline-flex items-center gap-1"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M9 3l-6 3v15l6-3 6 3 6-3V3l-6 3-6-3z"/><path d="M9 3v15M15 6v15"/></svg>Map</span>
              )}
            </button>
          ))}
        </div>
      ...
      {(activeCount > 0 || (!loading && resultCount != null)) && (
        <div className="mt-2.5 flex items-center gap-2 flex-wrap">
          ...
          {filters.city !== 'All' && (
            <ActiveTag label={`📍 ${filters.city}`} onRemove={() => update({ city: 'All' })} />
          )}
          {filters.category !== 'All' && (
            <ActiveTag
              label={`${CATEGORY_META[filters.category]?.emoji || ''} ${CATEGORY_META[filters.category]?.short || filters.category}`}
          ...
          <span className="ml-auto text-ztext3 text-xs">
            {loading ? 'Loading…' : `${resultCount} event${resultCount === 1 ? '' : 's'}`}
          </span>
        </div>
      )}
      ...
      {open && (
        <div className="mt-3 pt-4 border-t border-zborder space-y-4 animate-[fadeIn_.15s_ease]">
          <FilterSection label="When">
            {TIMEFRAMES.map(t => (
              <Chip key={t.id} active={filters.timeframe === t.id} onClick={() => update({ timeframe: t.id })}>
                {t.label}
              </Chip>
            ))}
          </FilterSection>
            ...
            {CATEGORIES.map(c => {
              ...
              return (
                <Chip key={c} active={filters.category === c} onClick={() => update({ category: c })}>
                  {m ? `${m.emoji} ${m.short}` : 'All categories'}
                </Chip>
              );
            })}
          </FilterSection>

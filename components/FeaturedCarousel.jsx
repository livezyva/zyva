...
export default function FeaturedCarousel({ events }) {
  ...
  const e = events[idx];
  const { dayLabel, time } = formatEventTime(e.start_datetime, e.end_datetime);
      ...
      <Link href={`/events/${e.slug}`} className="block relative aspect-[16/9] sm:aspect-[21/9]">
        ...
        <div className="relative z-10 h-full flex flex-col justify-end p-5 sm:p-10 max-w-3xl">
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="bg-black/60 backdrop-blur border border-zborder text-white text-xs px-2.5 py-1 rounded-full">{cat.emoji} {cat.short}</span>
            <span className="text-white text-xs sm:text-sm font-semibold">{dayLabel} · {time} · {e.city}</span>
          </div>
          <h2 className="font-headline text-2xl sm:text-4xl md:text-5xl font-bold tracking-tight leading-tight">
            {e.title}
          </h2>
          <p className="text-ztext2 text-sm sm:text-base mt-2 max-w-2xl line-clamp-2">{e.description}</p>
          <div className="mt-4 flex items-center gap-3">
            ...
            <span className="text-white/90 text-sm font-semibold">{e.price_label}</span>
          </div>
        </div>
      </Link>
